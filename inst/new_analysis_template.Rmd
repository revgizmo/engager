...
In it's current form, the `engager` library is useful for 4 related things:
...
4. **Analyze results** across students/speakers to gain insights into student engagement, with a particular focus on participation equity.
    ...
    3. `plot_users()` outputs plots for the key metrics with improved bar chart aesthetics.  This function needs to be run for each metric.
...
You can install the development version of engager like so:
...
# library(engager)
data_folder_input <- system.file("extdata", package = "engager")





transcripts_folder_input <- "test_transcripts"
...
### roster_file
roster_file_input <- "test_transcripts/ideal_course_roster.csv"
...
### names_lookup_file
names_lookup_file_input <- "example_section_names_lookup.csv"
...
instructor_name_input <- "Professor Smith"
...
student_summary_report_folder_input <- system.file("", package = "engager")
...
## 2. Load the engager library

+ `*r devtools::install_github("revgizmo/engager") `
+ `*r library(engager) `
    
```{r}

devtools::load_all()
# devtools::load_all('../engager')
# devtools::install_github("revgizmo/engager")
```
...
0. Prepare an institutionally authorized roster CSV outside the package source tree, or use the synthetic roster included with the package examples.
...
1. Run `plot_users()` for the key metrics to output said plots with improved bar chart aesthetics.

```{r fig.height=4}
# plot_users() - improved bar charts with minimal theme
...
```{r fig.height=4}
plot_users(students_only_transcripts_summary_df,
  metric = "session_ct", facet_by = "section", mask_by = "name"
)
plot_users(students_only_transcripts_summary_df,
  metric = "n", facet_by = "section", mask_by = "name"
)
plot_users(students_only_transcripts_summary_df,
  metric = "perc_n", facet_by = "section", mask_by = "name"
)
plot_users(students_only_transcripts_summary_df,
  metric = "duration", facet_by = "section", mask_by = "name"
)
plot_users(students_only_transcripts_summary_df,
  metric = "perc_duration", facet_by = "section", mask_by = "name"
)
plot_users(students_only_transcripts_summary_df,
  metric = "wordcount", facet_by = "section", mask_by = "name"
)
plot_users(students_only_transcripts_summary_df,
  metric = "perc_wordcount", facet_by = "section", mask_by = "name"
)
plot_users(students_only_transcripts_summary_df,
...
1. Run `plot_users()` with rank-based masking for the key metrics to output said plots with improved bar chart aesthetics and privacy protection.

```{r fig.height=4}
plot_users(
  data = students_only_transcripts_summary_df,
  metric = "session_ct", facet_by = "section", mask_by = "rank"
)
plot_users(
  data = students_only_transcripts_summary_df,
  metric = "n", facet_by = "section", mask_by = "rank"
)
plot_users(
  data = students_only_transcripts_summary_df,
  metric = "perc_n", facet_by = "section", mask_by = "rank"
)
plot_users(
  data = students_only_transcripts_summary_df,
  metric = "duration", facet_by = "section", mask_by = "rank"
)
plot_users(
  data = students_only_transcripts_summary_df,
  metric = "perc_duration", facet_by = "section", mask_by = "rank"
)
plot_users(
  data = students_only_transcripts_summary_df,
  metric = "wordcount", facet_by = "section", mask_by = "rank"
)
plot_users(
  data = students_only_transcripts_summary_df,
  metric = "perc_wordcount", facet_by = "section", mask_by = "rank"
)
plot_users(
  data = students_only_transcripts_summary_df,
  metric = "wpm", facet_by = "section", mask_by = "rank"
)
```
...
list.files(
  system.file(package = "engager")
)

list.files(
  system.file("extdata", package = "engager")
)
```
...
```{r}
load_zoom_recorded_sessions_list(data_folder = system.file("extdata", package = "engager"))
```
