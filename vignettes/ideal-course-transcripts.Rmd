...
The package provides batch processing functions to analyze all ideal course sessions simultaneously, making it easier to test and validate package functionality across multiple scenarios.
...
**Note**: The `export_ideal_transcripts_excel()` function temporarily exports to CSV format instead of Excel due to segfault issues with the openxlsx package. The function name is maintained for backward compatibility, but it creates CSV files with appropriate extensions. Summary and metadata sheets are created as separate CSV files when requested.
