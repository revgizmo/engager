...
For a course-level attendance workflow, provide an explicit roster and one row
per session. The analysis object contains local roster identifiers and is not a
shareable report. The report generator defaults to aggregate Markdown without
participant identifiers or transcript text.

```{r attendance-workflow}
...
The synthetic first session includes an unmatched guest. That speaker remains
separate from roster attendance and appears as a structured count for local
review. Masking and aggregation are technical privacy-supporting controls, not
guarantees of anonymity, compliance, or safe disclosure in every context.
...
5. **Multi-Session Attendance**: Analyze exact roster attendance across sessions
6. **Exporting**: Write privacy-supporting participation metrics, summaries, and reports
7. **Privacy Review**: Support technical review and institutional oversight
8. **Masking and Data Transformation**: Reduce exposure of recognized structured identifiers
...
Multi-session attendance reports are aggregate by default. The package does not
support longitudinal individual-student reporting or risk scoring.
