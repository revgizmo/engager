#!/usr/bin/env Rscript

#' Simple Comprehensive Performance Profiling Script
#' 
#' This script runs comprehensive performance profiling for weekly trend tracking.
#' Includes large synthetic datasets and detailed performance analysis.
#' 
#' @param output_dir Directory to save profiling results (default: "perf/reports/weekly")
#' @return List of comprehensive performance metrics
#' @export
run_simple_performance_profiling <- function(output_dir = "perf/reports/weekly") {
  
  # Set timeout for comprehensive profiling (10 minutes)
  options(timeout = 600)
  
  # Load required packages with error handling
  tryCatch({
    library(engager)
    library(jsonlite)
  }, error = function(e) {
    cat("ERROR: Failed to load required packages:", e$message, "\n")
    quit(status = 1)
  })
  
  # Create output directory
  dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
  
  cat("Running comprehensive performance profiling...\n")
  
  # Initialize results
  results <- list(
    timestamp = Sys.time(),
    environment = list(
      os = Sys.info()[["sysname"]],
      r_version = R.version.string,
      platform = R.version$platform,
      memory_limit = if (.Platform$OS.type == "windows") memory.limit() else "N/A"
    ),
    profiling = list()
  )
  
  # Test 1: Small transcript processing
  cat("Profiling small transcript processing...\n")
  transcript_dir <- system.file("extdata", "transcripts", package = "engager")
  small_files <- list.files(transcript_dir, pattern = "\\.(vtt|csv)$", full.names = TRUE)
  
  if (length(small_files) > 0) {
    small_file <- small_files[1]
    
    # Detailed profiling with multiple iterations
    small_times <- numeric(10)
    small_memory <- numeric(10)
    
    for (i in seq_len(10)) {
      start_time <- Sys.time()
      start_memory <- gc(verbose = FALSE)[1, "used"]
      
      result <- process_zoom_transcript(small_file)
      
      end_time <- Sys.time()
      end_memory <- gc(verbose = FALSE)[1, "used"]
      
      small_times[i] <- as.numeric(difftime(end_time, start_time, units = "secs")) * 1000
      small_memory[i] <- end_memory - start_memory
    }
    
    results$profiling$small_transcript <- list(
      time_stats = list(
        median_ms = median(small_times),
        mean_ms = mean(small_times),
        p95_ms = quantile(small_times, 0.95),
        min_ms = min(small_times),
        max_ms = max(small_times)
      ),
      memory_stats = list(
        median_mb = median(small_memory) / 1024^2,
        mean_mb = mean(small_memory) / 1024^2,
        peak_mb = max(small_memory) / 1024^2
      ),
      file_size_kb = file.size(small_file) / 1024
    )
  }
  
  # Test 2: Medium transcript processing
  if (length(small_files) > 1) {
    cat("Profiling medium transcript processing...\n")
    medium_file <- small_files[2]
    
    medium_times <- numeric(8)
    medium_memory <- numeric(8)
    
    for (i in seq_len(8)) {
      start_time <- Sys.time()
      start_memory <- gc(verbose = FALSE)[1, "used"]
      
      result <- process_zoom_transcript(medium_file)
      
      end_time <- Sys.time()
      end_memory <- gc(verbose = FALSE)[1, "used"]
      
      medium_times[i] <- as.numeric(difftime(end_time, start_time, units = "secs")) * 1000
      medium_memory[i] <- end_memory - start_memory
    }
    
    results$profiling$medium_transcript <- list(
      time_stats = list(
        median_ms = median(medium_times),
        mean_ms = mean(medium_times),
        p95_ms = quantile(medium_times, 0.95)
      ),
      memory_stats = list(
        median_mb = median(medium_memory) / 1024^2,
        peak_mb = max(medium_memory) / 1024^2
      ),
      file_size_kb = file.size(medium_file) / 1024
    )
  }
  
  # Test 3: Large synthetic dataset (O(n²) canary)
  cat("Profiling large synthetic dataset...\n")
  
  # Create large synthetic dataset
  large_data <- data.frame(
    user_name = rep(paste0("Student_", sprintf("%03d", 1:100)), 50),
    message = rep("This is a test message for performance profiling", 5000),
    timestamp = seq(as.POSIXct("2025-01-01 10:00:00"), length.out = 5000, by = "1 min")
  )
  
  large_times <- numeric(5)
  large_memory <- numeric(5)
  
  for (i in seq_len(5)) {
    start_time <- Sys.time()
    start_memory <- gc(verbose = FALSE)[1, "used"]
    
    # Simulate complex processing
    result <- large_data
    result$word_count <- nchar(result$message)
    result$hour <- as.numeric(format(result$timestamp, "%H"))
    result$engagement_score <- result$word_count * result$hour
    result$category <- rep(LETTERS[1:10], 500)
    result$processed <- result$engagement_score * result$hour
    
    end_time <- Sys.time()
    end_memory <- gc(verbose = FALSE)[1, "used"]
    
    large_times[i] <- as.numeric(difftime(end_time, start_time, units = "secs")) * 1000
    large_memory[i] <- end_memory - start_memory
  }
  
  results$profiling$large_synthetic <- list(
    time_stats = list(
      median_ms = median(large_times),
      mean_ms = mean(large_times),
      p95_ms = quantile(large_times, 0.95)
    ),
    memory_stats = list(
      median_mb = median(large_memory) / 1024^2,
      peak_mb = max(large_memory) / 1024^2
    ),
    dataset_size = nrow(large_data)
  )
  
  # Test 4: Memory stress test
  cat("Running memory stress test...\n")
  
  stress_times <- numeric(3)
  stress_memory <- numeric(3)
  
  for (i in seq_len(3)) {
    start_time <- Sys.time()
    start_memory <- gc(verbose = FALSE)[1, "used"]
    
    # Create and process multiple datasets
    datasets <- lapply(1:10, function(j) {
      data.frame(
        id = 1:1000,
        value = rnorm(1000),
        category = rep(LETTERS[1:10], 100)
      )
    })
    
    # Process all datasets
    processed_datasets <- lapply(datasets, function(df) {
      df$processed <- df$value * 2
      df$category_sum <- ave(df$value, df$category, FUN = sum)
      df$normalized <- (df$value - mean(df$value)) / sd(df$value)
      df
    })
    
    # Clean up
    rm(datasets)
    gc()
    
    end_time <- Sys.time()
    end_memory <- gc(verbose = FALSE)[1, "used"]
    
    stress_times[i] <- as.numeric(difftime(end_time, start_time, units = "secs")) * 1000
    stress_memory[i] <- end_memory - start_memory
  }
  
  results$profiling$memory_stress <- list(
    time_stats = list(
      median_ms = median(stress_times),
      mean_ms = mean(stress_times)
    ),
    memory_stats = list(
      median_mb = median(stress_memory) / 1024^2,
      peak_mb = max(stress_memory) / 1024^2
    )
  )
  
  # Save comprehensive results
  timestamp_str <- format(Sys.time(), "%Y%m%d_%H%M%S")
  output_file <- file.path(output_dir, paste0("profiling_", timestamp_str, ".json"))
  jsonlite::write_json(results, output_file, pretty = TRUE)
  
  # Save summary report
  summary_file <- file.path(output_dir, paste0("summary_", timestamp_str, ".txt"))
  writeLines(c(
    paste("Performance Profiling Summary -", Sys.time()),
    paste("Environment:", Sys.info()[["sysname"]], R.version.string),
    "",
    "Test Results:"
  ), summary_file)
  
  for (test_name in names(results$profiling)) {
    test <- results$profiling[[test_name]]
    write(c(
      paste("  ", test_name, ":"),
      paste("    Time (median):", sprintf("%.1f ms", test$time_stats$median_ms)),
      paste("    Memory (peak):", sprintf("%.1f MB", test$memory_stats$peak_mb)),
      ""
    ), summary_file, append = TRUE)
  }
  
  cat("Comprehensive profiling completed.\n")
  cat("Results saved to:", output_file, "\n")
  cat("Summary saved to:", summary_file, "\n")
  
  return(results)
}

# Run if called directly
if (!interactive()) {
  args <- commandArgs(trailingOnly = TRUE)
  output_dir <- if (length(args) > 0) args[1] else "perf/reports/weekly"
  
  # Run with error handling
  tryCatch({
    run_simple_performance_profiling(output_dir)
  }, error = function(e) {
    cat("ERROR: Performance profiling failed:", e$message, "\n")
    quit(status = 1)
  })
}
