...
set_ux_level <- function(level = "basic") {
  ...
  message("📊 Visible functions: ", visible_count)

  if (level == "basic") {
    message("💡 Tip: Use set_ux_level('intermediate') to see more functions")
  }
...
show_available_functions <- function(level = NULL) {
  ...
  cat("🎯 Available Functions (", level, " level)\n")
  ...
  if (length(common) > 0) {
    cat("📊 Common Functions (", length(common), "):\n")
    for (func in common) {
      desc <- UX_FUNCTION_DESCRIPTIONS[[func]]
      if (!is.null(desc)) {
        cat("  • ", func, "() - ", desc, "\n")
  ...
  if (length(advanced) > 0) {
    cat("⚙️ Advanced Functions (", length(advanced), "):\n")
  ...
  if (length(expert) > 0) {
    cat("🔧 Expert Functions (", length(expert), "):\n")
    for (func in expert) {
      cat("  • ", func, "()\n")
    }
    cat("\n")
  ...
  cat("💡 To see more functions: set_ux_level('", get_next_level(level), "')\n")
}
