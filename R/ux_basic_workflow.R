...
basic_transcript_analysis <- function(transcript_file, output_dir = "output", privacy_level = "high") {
  ...
  if (!file.exists(transcript_file)) {
    stop("❌ File not found: ", transcript_file, "\n",
  ...
  message("📄 File: ", basename(transcript_file))
  message("📁 Output: ", output_dir)
  message("🔒 Privacy: ", privacy_level)
  message("")
  ...
  tryCatch({
    ...
    message("💡 Try: show_getting_started() for help")
    message("💡 Or: show_function_help('basic_transcript_analysis')")
    stop(e)
  })
}
...
quick_analysis <- function(transcript_file) {
  ...
  message("📁 Results in: quick_output/")
...
batch_basic_analysis <- function(transcript_files, output_dir = "batch_output", privacy_level = "high") {
  ...
  if (length(transcript_files) == 0) {
    stop("❌ No transcript files provided")
  }
  ...
  message("🔄 Batch Analysis Mode")
  message(paste(rep("=", 25), collapse = ""))
  message("📄 Files: ", length(transcript_files))
  message("📁 Output: ", output_dir)
  message("🔒 Privacy: ", privacy_level)
  message("")
  ...
  for (i in seq_along(transcript_files)) {
    ...
    tryCatch({
      ...
      message("✅ Completed: ", basename(file))
    }, error = function(e) {
      message("❌ Failed: ", basename(file), " - ", e$message)
  ...
  message("📊 Results saved to: ", output_dir)
  message("📈 Successful: ", sum(sapply(results, function(x) is.null(x$error))))
  message("❌ Failed: ", sum(sapply(results, function(x) !is.null(x$error))))
