...
summarize_transcript_files <-
  function(transcript_file_names = NULL,
    ...
    if (tibble::is_tibble(transcript_file_names) &&
      ...
      original_metadata <- extract_original_metadata(transcript_file_names, preserve_metadata)

      # Process transcript files
      all_results <- process_transcript_files(transcript_file_names, transcripts_folder_path, names_to_exclude)
...
# Helper function to extract original metadata
extract_original_metadata <- function(transcript_file_names, preserve_metadata) {
...
# Helper function to process transcript files
process_transcript_files <- function(transcript_file_names, transcripts_folder_path, names_to_exclude) {
  # Use base R operations instead of dplyr to avoid segmentation fault
...
process_and_return_results <- function(all_results, preserve_metadata, original_metadata) {
